const addDirection = (move: string) => {
...
export const scrambler3x3 = (move_count: number) => {
	const getMove = (last_move: string): string => {
		const options = ["L", "R", "U", "D", "F", "B"];
		let move: string = options[Math.floor(Math.random() * options.length)]!;
